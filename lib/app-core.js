...
function _fjUnfulfilledPlaceholderHtml() {
  return `<div class="today-card">
...
function buildUnfulfilledHtml(lines) {
  if (!lines.length) return _fjUnfulfilledPlaceholderHtml(); // no FJ data at all
...
function exportOrderXlsx() {
  const rows = Object.keys(orderItems).map(id => {
    const p = PRODUCTS.find(x => x.id === id);
    if (!p) return null;
    const qty = orderItems[id];
    const tierIdx = qty >= 11 ? 2 : qty >= 6 ? 1 : 0;
    const price = tierPrice(p.priceUnit, tierIdx);
    return { Product: p.name, Unit: p.unit, Qty: qty, 'Unit Price': price, 'Line Total': price * qty };
  }).filter(Boolean);
  _exportRows(rows, 'order');
}
...
// above — given a catalog product, find every customer (company-wide, every
// rep) whose purchase history resolves to it. Same ~40-50% recall heuristic
// as getPitchSuggestions' use of _findCatalogMatch; this is a known,
// pre-existing limitation of the underlying data (see comment above
...
function findCustomersForProduct(product) {
  ...
  const byCustomer = new Map(); // key: sales+'|'+name -> row
  _flattenAllCustomerEntries().forEach(c => {
    (c.products || []).forEach(line => {
      const match = _findCatalogMatch(line.name);
      if (!match || match.id !== product.id) return;
      const key = c.sales + '|' + c.name;
      const row = byCustomer.get(key) || { customer: c.name, area: c.area, rep: c.sales, qty: 0, value: 0, lastMonthIdx: -1, lastMonth: '' };
      row.qty += line.qty || 0;
      row.value += line.value || 0;
      const mi = monthIdx(line.month);
      if (mi >= row.lastMonthIdx) { row.lastMonthIdx = mi; row.lastMonth = line.month; }
      byCustomer.set(key, row);
    });
  });
  const rows = [...byCustomer.values()].sort((a, b) => b.value - a.value);
...
function buildSoldToHtml(rows) {
  if (!rows.length) {
    return `<div class="pm-sold-title">Sold To</div>
      <div class="tc-coming-soon">No purchase history found for this product yet</div>`;
  }
  const CAP = 25;
  const shown = rows.slice(0, CAP);
  const hidden = rows.length - shown.length;
  let h = `<div class="pm-sold-title">Sold To <span style="font-weight:400;color:var(--txt3);font-size:.68rem">(${rows.length} · approximate, based on order history)</span></div>
    <div class="tc-so">`;
  shown.forEach(r => {
    h += `<div class="tc-so-group"><div class="tc-so-cust-hdr"><div class="tc-so-cust">${escHtml(r.customer)}</div><div class="tc-so-cust-total">${fmtShort(r.value)}</div></div>
      <div class="tc-so-row"><div class="tc-so-prod">${escHtml(r.rep)} · ${escHtml(r.area || '')}</div><div class="tc-so-qty">${fmtNum(r.qty)} · last ${escHtml(r.lastMonth || '—')}</div></div>
    </div>`;
  });
  if (hidden > 0) h += `<div class="tc-coming-soon" style="padding:8px">+${hidden} more customers not shown, sorted by value</div>`;
  h += `</div>`;
  return h;
}
...
function renderOrder() {
  ...
  body.innerHTML = `
    ...
    <div class="order-footer">
      ...
      <button class="of-draft" onclick="exportOrderXlsx()" title="Export to Excel">${uiIcon('download',18)}</button>
