...
function buildUnfulfilledHtml(lines) {
  ...
  let h = `<div class="today-card tc-unfulfilled">
    <div class="tc-title tc-title-lost">⚠️ Unfulfilled · ${fmtShort(totalLost)} lost (${unfulfilled.length} items)</div>
    <div class="tc-so">`;
  custOrder.forEach(cust => {
    const c = custMap[cust];
...
function buildLogin() {
  ...
  const saved = localStorage.getItem('mku_saved_rep');
  if (saved !== null) {
...
function selectRep(idx) {
  try {
    ...
    localStorage.setItem('mku_saved_rep', String(idx));
...
function logout() {
  ...
  orderItems = {};
  localStorage.removeItem('mku_saved_rep');
...
function renderStock() {
  ...
  const btnLabels = { ok:'✅ In Stock', low:'⚠️ Low', critical:'🔴 Critical', out:'❌ Out' };
  Object.keys(btnLabels).forEach(k => {
    const btn = document.querySelector(`.sf.${k}`);
    if (btn) btn.textContent = `${btnLabels[k]} ${counts[k]}`;
  });
...
function renderCustomers() {
  ...
  el.innerHTML = custs.map(c => {
    ...
    const inactiveTag = mAgo >= 2 ? ` <span style="font-size:.6rem;background:var(--red-l);color:var(--red);padding:1px 5px;border-radius:4px;font-weight:700">⚠️ ${mAgo}mo</span>` : '';
