...
function buildLogin() {
  ...
  // Add optgroups
  const fieldReps = REP_CONFIG.filter(r => !r.area.includes('Nestlé') && r.area !== 'Management');
  const nestleReps = REP_CONFIG.filter(r => r.area.includes('Nestlé'));
  const mgmtReps = REP_CONFIG.filter(r => r.area === 'Management');
  ...
  waitForGlobal('AppAuth').then(() => window.AppAuth.restoreSession()).then(repRow => {
    if (repRow) {
      const idx = REP_CONFIG.findIndex(r => r.name === repRow.name);
      if (idx !== -1) {
        selectRep(idx, { repId: repRow.id, authUserId: repRow.auth_user_id, isManager: repRow.is_manager, phone: repRow.phone, photoUrl: repRow.photo_url });
        return;
      }
    }
    // No valid session — pre-select the dropdown as a UX convenience only;
    // this no longer skips authentication, it just opens the PIN pad.
...
async function finishPinChange() {
  ...
  try {
    ...
    _pinBuffer = '';
    document.getElementById('login-pin-name').textContent = 'Choose a new PIN';
    document.getElementById('login-pin-error').textContent = "Couldn't save new PIN. Try again.";
    renderPinDots();
    setPinKeypadDisabled(false);
  }
}
...
function selectRep(idx, authInfo) {
  try {
    ...
    if (currentRep.isManager) { switchTab('target'); }
    // Order's customer-suggestion panel needs a personal customer portfolio
    // a Manager doesn't have, so hide the tab rather than leave it half-functional.
    document.getElementById('bnav-order').classList.toggle('hidden', !!currentRep.isManager);
...
function toggleAnnouncements() {
  ...
  if (overlay.classList.contains('hidden')) {
    ...
    if (newBtn) newBtn.classList.toggle('hidden', !currentRep?.isManager);
...
function openSendAnnouncement() {
  ...
  targetSel.innerHTML = '<option value="">Everyone</option>' +
    REP_CONFIG.filter(r => r.name !== 'Manager')
...
function initCustomers() {
  ...
  if (contactsBtn) contactsBtn.classList.toggle('hidden', !currentRep?.isManager);
...
async function onProfilePhotoSelected(file) {
  ...
  try {
    ...
    errEl.textContent = 'Could not upload photo — ' + (e.message || e);
  }
}
...
async function finishChangePin() {
  try {
    ...
    _cpBuffer = '';
    document.getElementById('change-pin-title').textContent = 'Choose a new PIN';
    document.getElementById('change-pin-error').textContent = "Couldn't save new PIN. Try again.";
    renderChangePinDots();
  }
}
...
async function submitIntelForm() {
  ...
  try {
    ...
    if (_editingIntelLogId) {
      ...
      await window.CompetitorIntel.createLog({
