...
// LOGIN
// ════════════════
// lib/auth.js is a <script type="module"> that imports the Supabase SDK
// from a CDN — that network fetch can outlast the local data-load chain
// that triggers _showLogin(), so window.AppAuth isn't guaranteed to exist
// yet when buildLogin()/submitPin() first run. Poll briefly instead of
// assuming module-script load order.
function waitForAppAuth() {
  if (window.AppAuth) return Promise.resolve();
  return new Promise(resolve => {
    const check = () => window.AppAuth ? resolve() : setTimeout(check, 20);
    check();
  });
}
...
function buildLogin() {
  ...
  waitForAppAuth().then(() => window.AppAuth.restoreSession()).then(repRow => {
    if (repRow) {
