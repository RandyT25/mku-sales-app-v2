...
function selectRep(idx, authInfo) {
  try {
    ...
    if (currentRep.bisdevCategory) { openBisdevDashboard(currentRep.bisdevCategory); }
    if (currentRep.isNestleCoordinator) { openNestleCoordinatorDashboard(); }
...
function renderSuperAdminPanel() {
  ...
  const groups = [
    ['Field Sales', REP_CONFIG.filter(r => !r.area.includes('Nestlé') && r.div !== 'MGMT')],
    ['Nestlé Team', REP_CONFIG.filter(r => r.area.includes('Nestlé'))],
    ['Management',  REP_CONFIG.filter(r => r.div === 'MGMT')],
  ];
