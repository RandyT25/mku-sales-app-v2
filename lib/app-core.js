...
const REP_CONFIG = [
  ...
  { name:'Vacant',         area:'KUTA - INDUSTRI / HOTEL',div:'MKS' },
  ...
  { name:'Vacant',         area:'NP-2 (Nestlé)',          div:'MKS' },
...
function buildTomorrowDeliveryHtml(lines) {
  if (isFJStale(lines)) {
    return `<div class="today-card">
      <div class="tc-title tc-title-tomorrow">${uiIcon('truck',14)} Tomorrow's Deliveries</div>
...
function buildUnfulfilledHtml(lines) {
  ...
  const lostLines = unfulfilled.map(r => {
    const { value: lost, tier: lostTier } = _fjLookupRevenue(r.no_so, r.kode_brg, r.nama_brg, r.qty_so, r.satuan);
    totalLost += lost;
    return { ...r, lost_rev: lost, lostTier };
  });
  // Group by customer
  const custMap = {}, custOrder = [];
  lostLines.forEach(r => {
  ...
  custOrder.forEach(cust => {
    ...
    c.items.forEach(r => {
      ...
      h += `<div class="tc-so-row"><div class="tc-so-prod">${r.nama_brg}</div><div class="tc-so-qty">${fmtNum(r.qty_so)} ${r.satuan}</div><div class="tc-so-rev${isEstimate && r.lost_rev > 0 ? ' tc-so-rev-est' : ''}">${lostHtml}</div></div>`;
    });
    h += `</div>`;
  });
  h += `</div></div>`;
...
function renderManagementRecap() {
  ...
  if (nestle_target > 0) miniCardsHtml +=
    ...
    '<div class="th-mini-pct" style="color:' + pctColor(Math.round(nestle_ach/nestle_target*100)) + '">' + Math.round(nestle_ach/nestle_target*100) + '%</div></div>';
  ...
  bodyEl.innerHTML = milestoneHtml + `
    ...
    <div class="area-card">
      <div class="ac-title">Team Leaderboard</div>
      ${teamHtml}
    </div>
...
function getBadgeIcons(repName, d) {
  return getBadges(repName, d).map(b => uiIcon(b.icon, 13)).join('');
}

// ── REP TYPE HELPERS ──
const NESTLE_REPS  = ['Ridwan','Gek Mas'];
  ...
  bodyHtml += `<div class="area-card">
    ...
    ${sortedNestle.map((n,i) => {
      ...
      return `<div class="ac-row" style="${isMe?'background:var(--red-l);border-radius:8px;padding:8px 6px;margin:-2px -4px;':''}">
        <div class="ac-rank" style="${i<3?'color:var(--gold);font-weight:700':''}">${i+1}</div>
        ${avatarHtml(n.sales, repColorByName(n.sales), 26)}
        <div style="flex:1;min-width:0">
          <div class="ac-name">${n.sales}${isMe?' <span class="ac-you">You</span>':''}${nb?' <span class="ac-badges">'+nb+'</span>':''}</div>
...
function renderNestleCoordinatorTarget(heroElId, bodyElId) {
  ...
  const leaderRow = (a, i) => `<div class="ac-row">
    <div class="ac-rank" style="${i<3?'color:var(--gold);font-weight:700':''}">${i+1}</div>
    ${avatarHtml(a.sales, repColorByName(a.sales), 26)}
    <div style="flex:1;min-width:0">
      <div class="ac-name">${escHtml(a.sales)}</div>
...
function renderSuperAdminPanel() {
  ...
  // Excludes Super Admin's own entry — viewing "as yourself" is meaningless
  // since you're already looking at this exact panel.
  const others = REP_CONFIG.filter(r => r.name !== currentRep.name);
  const groups = [
  ...
  bodyEl.innerHTML = groups.map(([label, reps]) => `
    <div class="area-card">
      <div class="ac-title">${label}</div>
      ${reps.map(r => {
        const idx = REP_CONFIG.indexOf(r);
        return `<div class="ac-row" style="cursor:pointer" onclick="viewAsRep(${idx})">
...
async function viewAsRep(idx) {
  const target = REP_CONFIG[idx];
  try {
    await waitForGlobal('AppAuth');
    const repRow = await window.AppAuth.fetchRepForViewAs(target.name);
    ...
    document.getElementById('view-as-name').textContent = target.name;
    document.getElementById('view-as-banner').classList.remove('hidden');
  } catch (e) {
    console.error('viewAsRep error:', e);
    alert("Couldn't load that profile — it may not have a rep row yet (e.g. a Vacant slot, or not seeded/migrated yet).");
  }
}
...
async function editRepProfile(idx) {
  ...
  try {
    ...
    document.getElementById('edit-rep-role').value = repRow.is_manager ? 'manager'
      ...
      : 'none';
    document.getElementById('edit-rep-email').textContent = window.AppAuth.deriveLoginAlias(target.name);
    ...
    alert("Couldn't load that profile — it may not have a rep row yet (e.g. a Vacant slot, or not seeded yet).");
  }
}
...
function loadDraft(id) {
  ...
  if (!draft) return;
  if (Object.keys(orderItems).length && !confirm('Replace the current order with this draft?')) return;
...
function isNestleStockItem(s) {
  ...
  return PRODUCTS.some(p => p.category === 'Nestle' && p.name.toLowerCase().includes(sPrefix));
}
...
let _intelView = 'recent'; // 'recent' | 'summary'
...
async function deleteIntelLogConfirm(id) {
  if (!confirm('Delete this competitor price entry?')) return;
  try {
    await waitForGlobal('CompetitorIntel');
