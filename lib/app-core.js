...
async function submitPin() {
  ...
  try {
    ...
    selectRep(idx, { repId: repRow.id, authUserId: repRow.auth_user_id, isManager: repRow.is_manager, phone: repRow.phone, photoUrl: repRow.photo_url, bisdevCategory: repRow.bisdev_category });
  } catch (e) {
...
function selectRep(idx, authInfo) {
  try {
    ...
    // Bisdev login: same company-wide-only situation as Manager, but the
    // dashboard is presented as a popup that opens immediately on login
    // rather than requiring a tap into the Target tab.
    ...
    // a Manager/Bisdev rep doesn't have, so hide the tab rather than leave it half-functional.
    document.getElementById('bnav-order').classList.toggle('hidden', !!(currentRep.isManager || currentRep.bisdevCategory));
