...
async function submitPin() {
  ...
  try {
    ...
    selectRep(idx, { repId: repRow.id, authUserId: repRow.auth_user_id, isManager: repRow.is_manager, phone: repRow.phone, photoUrl: repRow.photo_url, bisdevCategory: repRow.bisdev_category, isNestleCoordinator: repRow.is_nestle_coordinator });
  } catch (e) {
...
function selectRep(idx, authInfo) {
  try {
    ...
    // a Manager/Bisdev/Coordinator rep doesn't have, so hide the tab rather than leave it half-functional.
    document.getElementById('bnav-order').classList.toggle('hidden', !!(currentRep.isManager || currentRep.bisdevCategory || currentRep.isNestleCoordinator));
