...
function buildLogin() {
  ...
  waitForGlobal('AppAuth').then(() => window.AppAuth.restoreSession()).then(repRow => {
    if (repRow) {
      ...
      if (idx !== -1) {
        selectRep(idx, { repId: repRow.id, authUserId: repRow.auth_user_id, isManager: repRow.is_manager });
        return;
      }
    }
...
function populateOrderCustDatalist() {
  const dl = document.getElementById('order-cust-datalist');
  if (!dl) return;
  dl.innerHTML = getRepCustomerList().map(c => `<option value="${escHtml(c.name)}">`).join('');
}

function onOrderCustInput(name) {
  const trimmed = name.trim().toLowerCase();
  const panel = document.getElementById('order-suggestions');
  if (!trimmed) { panel.classList.add('hidden'); panel.innerHTML = ''; return; }
  const entry = getRepCustomerList().find(c => c.name.trim().toLowerCase() === trimmed);
...
function openIntelForm() {
  ['intel-competitor','intel-product','intel-category','intel-price','intel-unit','intel-customer','intel-notes']
