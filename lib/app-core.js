...
const REP_CONFIG = [
  ...
  // app recognize all of them as the same rep.
  { name:'Ronald',         area:'KUTA - HOTEL',           div:'MKS', dbName:'Lani',
    aliases:['VACANT','Vacant Hotel Kuta Seminyak','Vacant Hotel Seminyak Kuta','Lani'] },
  { name:'Monica',         area:'KUTA SEL - NUSA DUA',    div:'MKS' },
  { name:'Sujana',         area:'SEMINYAK',                div:'MKS' },
  { name:'Wahyu',          area:'KUTA - LEGIAN / INDUSTRI',div:'MKS', dbName:'Eka', aliases:['EKA/WAHYU', 'WAHYU FIB', 'EKA FIB', 'Eka'] },
  ...
  { name:'Widi',           area:'NP-2 (Nestlé)',          div:'MKS', dbName:'Redi', aliases:['Redi', 'Vacant'] },
  ...
  { name:'Bisdev Bakery',    area:'Company-wide (Bakery)',    div:'MGMT' },
...
function salesNameMatches(feedName, repName, aliases) {
  ...
  const feed = feedName.toLowerCase().trim();
  if (feed === repName.toLowerCase().trim()) return true;
  if (aliases && aliases.some(a => a.toLowerCase().trim() === feed)) return true;
  return false;
}

// Groups raw spreadsheet rows into a single aggregated row per Rep based on REP_CONFIG aliases
function aggregateMainAreas(rawAreas) {
  const aggregated = [];
  const handledRepNames = new Set();
  
  rawAreas.forEach(a => {
    // Find if this row matches a known rep
    const cfg = REP_CONFIG.find(r => salesNameMatches(a.sales, r.name, r.aliases));
    
    if (cfg) {
      if (handledRepNames.has(cfg.name)) return; // already aggregated this rep
      handledRepNames.add(cfg.name);
      
      // Find all rows matching this rep
      const myAreas = rawAreas.filter(ra => salesNameMatches(ra.sales, cfg.name, cfg.aliases));
      let ft=0, bt=0, bkt=0, fa=0, ba=0, bka=0;
      myAreas.forEach(ra => { 
        ft += ra.food_target||0; fa += ra.food_ach||0; 
        bt += ra.bev_target||0; ba += ra.bev_ach||0; 
        bkt+= ra.bakery_target||0; bka+= ra.bakery_ach||0; 
      });
      const mt = ft + bt + bkt;
      const ma = fa + ba + bka;
      aggregated.push({
        sales: cfg.name,
        area: cfg.area,
        food_target: ft, food_ach: fa,
        bev_target: bt, bev_ach: ba,
        bakery_target: bkt, bakery_ach: bka,
        pct: mt > 0 ? Math.round((ma / mt) * 100) : 0
      });
    } else {
      // Unmapped row, keep it as-is
      aggregated.push(a);
    }
  });
  return aggregated;
...
// currentRep holds the currently active rep configuration for both
// the login PIN screen and the main app dashboard.
...
async function submitPin() {
  ...
  try {
    ...
    const loginName = target.dbName || target.name;
    const repRow = await window.AppAuth.login(loginName, pin);
...
function renderManagementRecap() {
  ...
  let food_target = 0, bev_target = 0, bakery_target = 0, food_ach = 0, bev_ach = 0, bakery_ach = 0;
  mainAreas.forEach(a => {
    food_target   += a.food_target   || 0;
    bev_target    += a.bev_target    || 0;
    bakery_target += a.bakery_target || 0;
    food_ach      += a.food_ach      || 0;
    bev_ach       += a.bev_ach       || 0;
    bakery_ach    += a.bakery_ach    || 0;
  });
  ...
  // Naughty Nuris is structurally Food revenue — fold it into the main Food
  // total so Overall %/Food match the official "DATA PENCAPAIAN — GLOBAL"
  // report. Still shown as its own nested sub-row under the owning rep in
  // Team Leaderboard (nnSubRow, below).
  nnAreas.forEach(a => {
    food_target += a.food_target || 0;
    food_ach    += a.food_ach    || 0;
  });
  const main_target = food_target + bev_target + bakery_target + nestle_target;
  const main_ach = food_ach + bev_ach + bakery_ach + nestle_ach;
  ...
  // Mini breakdown cards: Food / Beverage / Bakery / Nestlé
  let miniCardsHtml = '';
  ...
  if (bakery_target > 0) miniCardsHtml +=
    '<div class="th-mini"><div class="th-mini-label">Bakery</div>' +
    '<div class="th-mini-val">' + fmtShort(bakery_ach) + '</div>' +
    '<div class="th-mini-pct" style="color:' + pctColorOnDark(Math.round(bakery_ach/bakery_target*100)) + '">' + Math.round(bakery_ach/bakery_target*100) + '%</div></div>';
  ...
  const _trendItems = Object.entries(_byMonth).slice(-4).map(([, dt]) => {
    ...
    if (!_da) return null;
    const _mainA = _da.area_targets || [];
    let _ft=0,_bt=0,_bkt=0,_fa=0,_ba=0,_bka=0;
    _mainA.forEach(a => { _ft+=a.food_target||0; _bt+=a.bev_target||0; _bkt+=a.bakery_target||0; _fa+=a.food_ach||0; _ba+=a.bev_ach||0; _bka+=a.bakery_ach||0; });
    let _nt=0,_na=0;
    (_da.nestle_areas||[]).forEach(n => { _nt+=n.target||0; _na+=n.achievement||0; });
    const _mt = _ft+_bt+_bkt+_nt, _ma = _fa+_ba+_bka+_na;
  ...
  const nnSubRow = (a) => {
    const cfg = REP_CONFIG.find(r => r.name === a.sales);
    const nnRow = nnAreas.find(n => salesNameMatches(n.sales, a.sales, cfg?.aliases));
    if (!nnRow) return '';
  ...
  const team = aggregateMainAreas(mainAreas).sort((a,b) => b.pct - a.pct);
  ...
  // Visual milestone progress bars — Food / Beverage / Bakery / Nestlé
  const _bars = [];
  ...
  if (bakery_target > 0) _bars.push({ label:'Bakery',        ach:bakery_ach, tgt:bakery_target, color:'#8B5A2B' });
  ...
  _lastLeaderboardRows = { team, nestle, nn: nnAreas };
}
let _lastLeaderboardRows = { team: [], nestle: [], nn: [] };
...
function exportLeaderboardXlsx() {
  ...
  const rows = [
    ..._lastLeaderboardRows.team.map(a => ({
      ...
      'Bakery Target': a.bakery_target||0, 'Bakery Achieved': a.bakery_ach||0,
    })),
...
function getBadges(repName, d, aliases) {
  ...
  // Get all areas for this rep — includes Naughty Nuris, folded in as Food
  // revenue same as renderTarget()'s Overall %.
  const myAreas = d.area_targets.filter(a =>
    salesNameMatches(a.sales, repName, aliases)
  );
  ...
  myAreas.forEach(a => {
    total_t += (a.food_target||0) + (a.bev_target||0) + (a.bakery_target||0);
    total_a += (a.food_ach||0) + (a.bev_ach||0) + (a.bakery_ach||0);
  });
  if (nestleEntry) {
...
function isNestleRep(name) {
  if (!name) return false;
  const lowerName = name.toLowerCase().trim();
  if (typeof currentRep !== 'undefined' && currentRep && lowerName === currentRep.name.toLowerCase().trim() && currentRep.dbName) {
    if (NESTLE_REPS.some(r => r.toLowerCase().trim() === currentRep.dbName.toLowerCase().trim())) return true;
  }
  return NESTLE_REPS.some(r => r.toLowerCase().trim() === lowerName);
}
function hasNNArea(name) { 
  if (!name) return false;
  const lowerName = name.toLowerCase().trim();
  return Object.keys(NN_REPS).some(k => k.toLowerCase().trim() === lowerName); 
}
...
function feedDisplayName(feedName) {
  ...
  const feedLower = feedName.toLowerCase().trim();
  const cfg = REP_CONFIG.find(r => r.dbName && r.dbName.toLowerCase().trim() === feedLower);
...
function renderTarget() {
  ...
  // Naughty Nuris is structurally Food revenue — fold it into food_target/
  // food_ach so this rep's Overall %/Food total match the official report.
  // Still shown as its own nested sub-row in the Area Leaderboard below.
  if (myNNArea) {
    food_target += myNNArea.food_target || 0;
    food_ach    += myNNArea.food_ach    || 0;
  }

  const main_target  = food_target + bev_target + bakery_target;
  const main_ach     = food_ach + bev_ach + bakery_ach;
  const overall_pct  = main_target > 0 ? Math.round((main_ach / main_target) * 100) : 0;
  ...
  if (multiArea) {
    ...
    myMainAreas.forEach(a => {
      const at = (a.food_target||0) + (a.bev_target||0) + (a.bakery_target||0);
      const aa = (a.food_ach||0)   + (a.bev_ach||0)   + (a.bakery_ach||0);
  ...
  const _trendItems = Object.entries(_byMonth).slice(-4).map(([, dt]) => {
    const _da = RAW.targets_by_date[dt];
    if (!_da) return null;
    const _ar = (_da.area_targets||[]).filter(a => salesNameMatches(a.sales, repName, repAliases));
    if (!_ar.length) return null;
    let _mt = 0, _ma = 0;
    _ar.forEach(a => { _mt += (a.food_target||0)+(a.bev_target||0)+(a.bakery_target||0); _ma += (a.food_ach||0)+(a.bev_ach||0)+(a.bakery_ach||0); });
    if (_mt <= 0) return null;
    const _ap = Math.round(_ma/_mt*100);
    return `<div class="th-trend-item" style="color:${pctColor(_ap)}">${dt.slice(5,7)}/${dt.slice(2,4)}<br><b>${_ap}%</b></div>`;
  }).filter(Boolean);
  ...
  if (multiArea) {
    ...
    // Progress to Milestones shows NN as its own bar (below), so back its
    // numbers out of the Food bar here — unlike the Overall %/Food headline
    // above, which deliberately keeps NN folded in to match the official
    // "DATA PENCAPAIAN — GLOBAL" report (see fold-in note near food_target).
    const nnBarTarget = myNNArea ? (myNNArea.food_target || 0) : 0;
    const nnBarAch     = myNNArea ? (myNNArea.food_ach    || 0) : 0;
    const foodOnlyTarget = food_target - nnBarTarget;
    const foodOnlyAch    = food_ach - nnBarAch;
    if (foodOnlyTarget > 0) bars.push({ label:'Food',     ach:foodOnlyAch, tgt:foodOnlyTarget, color:'#AD1F28' });
    if (bev_target > 0)     bars.push({ label:'Beverage', ach:bev_ach,     tgt:bev_target,     color:'#0F2E5C' });
    if (bakery_target > 0)  bars.push({ label:'Bakery',   ach:bakery_ach,  tgt:bakery_target,  color:'#8B5A2B' });
    if (nnBarTarget > 0)    bars.push({ label:'Naughty Nuris', ach:nnBarAch, tgt:nnBarTarget,  color:'#B07D1A' });
  }
...
function renderBisdevTarget(category, heroElId, bodyElId) {
  ...
  const mainAreas = d.area_targets.filter(a => !a.area.includes('NAUGHTY NURIS'));
  const nnAreas   = d.area_targets.filter(a =>  a.area.includes('NAUGHTY NURIS'));
  // Bisdev reps cover one category company-wide — Food/Beverage are the
  // established roles; Bakery is a newer peer category (see the per-area
  // bakery_target/bakery_ach fields folded into Food/Beverage totals
  // elsewhere) that can be assigned the same way once a Bisdev Bakery rep
  // exists (reps.bisdev_category currently only allows 'food'/'beverage' —
  // extending that CHECK constraint is a separate DB migration).
  const CATEGORY_META = {
    food:     { targetKey: 'food_target',   achKey: 'food_ach',   label: 'Food',     barColor: '#AD1F28' },
    beverage: { targetKey: 'bev_target',    achKey: 'bev_ach',    label: 'Beverage', barColor: '#0F2E5C' },
    bakery:   { targetKey: 'bakery_target', achKey: 'bakery_ach', label: 'Bakery',   barColor: '#8B5A2B' },
  };
  const { targetKey, achKey, label, barColor } = CATEGORY_META[category] || CATEGORY_META.food;
  ...
  // Naughty Nuris is structurally Food revenue — fold it in for the Food
  // view only (NN rows carry no beverage target/achievement). Still shown
  // as its own nested sub-row in the leaderboard below.
  if (category === 'food') {
    nnAreas.forEach(a => { target += a.food_target || 0; ach += a.food_ach || 0; });
  }
  ...
  const catPct = a => {
    ...
    if (category === 'food') {
      ...
      const nnRow = nnAreas.find(n => salesNameMatches(n.sales, a.sales, cfg?.aliases));
      if (nnRow) {
        t += nnRow.food_target || 0;
        ac += nnRow.food_ach || 0;
      }
    }
    return t > 0 ? Math.round((ac / t) * 100) : 0;
  };
  ...
  // Naughty Nuris sub-row, nested under its owning rep's row — same visual
  // pattern as renderManagementRecap()'s nnSubRow, shown for Food only since
  // that's the only category NN contributes to.
  const nnSubRow = (a) => {
    const cfg = REP_CONFIG.find(r => r.name === a.sales);
    const nnRow = nnAreas.find(n => salesNameMatches(n.sales, a.sales, cfg?.aliases));
    if (!nnRow) return '';
  ...
  // Group rows by Rep so we don't display two Wahyu rows
  const team = aggregateMainAreas(mainAreas).sort((a, b) => catPct(b) - catPct(a));
  const teamHtml = team.map((a, i) => bisdevLeaderRow(a, i) + (category === 'food' ? nnSubRow(a) : '')).join('');
...
function openBisdevDashboard(category) {
  const label = category === 'food' ? 'Food' : category === 'bakery' ? 'Bakery' : 'Beverage';
  document.getElementById('bisdev-title').textContent = label + ' — Company Overview';
...
function prefetchCatalogBlobs() {
  ['MKU Catalog 2026.pdf', 'MKS Catalog 2026.pdf', 'Nestle Brochure.pdf'].forEach(filename => {
...
function getRepCustomerList() {
  ...
  let entries = [];
  // Company-wide roles (Manager, Super Admin, Bisdev Food/Beverage/Bakery)
  // have no personal customer portfolio, so they all get the full,
  // deduped, company-wide list — same branch that already covered Manager.
  // Bisdev doesn't get a division-filtered subset because the source data
  // has no reliable per-customer food/beverage/bakery signal (purchase
  // history's `category` field comes back empty in production; the Bisdev
  // dashboards' Food/Beverage/Bakery split only exists at the area-target
  // aggregate level, not per customer).
  if (!rep || rep === 'Management Bali' || currentRep?.isManager || currentRep?.isSuperAdmin || currentRep?.bisdevCategory) {
    ...
    return `<div class="cust-card" onclick="openCustModal('${escHtml(c.sales)}','${escHtml(c.code)}')">
      <div class="cust-card-top">
        ...
        <div class="cust-card-info">
          ...
          <div class="cust-card-sub">${escHtml(c.area||'')} · ${escHtml(c.group||'')}${(currentRep?.isManager || currentRep?.isSuperAdmin || currentRep?.bisdevCategory) ? ' · '+escHtml(c.sales||'') : ''}</div>
        </div>
...
function openProfile() {
  ...
  // If Super Admin is viewing another rep, hide the Change PIN button
  // so they don't accidentally change their own Super Admin PIN.
  const changePinBtn = document.getElementById('change-pin-btn');
  const resetPinBtn = document.getElementById('reset-rep-pin-btn');
  const isViewAs = _superAdminOwnIdx !== null && _superAdminOwnIdx !== REP_CONFIG.indexOf(currentRep);
  if (changePinBtn) changePinBtn.style.display = isViewAs ? 'none' : '';
  if (resetPinBtn) resetPinBtn.style.display = isViewAs ? '' : 'none';
...
window.resetRepPinProfile = function() {
  resetRepPin(REP_CONFIG.indexOf(currentRep));
};
