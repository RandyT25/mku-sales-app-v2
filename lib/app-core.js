...
// but spanning every rep (call with getFJData(null, true)) and grouped by sales
// rep first, then by customer, so a manager can see whose orders need chasing.
...
function renderManagementRecap() {
  ...
  bodyEl.innerHTML += buildTomorrowDeliveryByRepHtml(getFJData(null, false));
  bodyEl.innerHTML += buildUnfulfilledByRepHtml(getFJData(null, true));
