...
window.resetRepPin = async function() {
  if (!confirm(`Are you sure you want to reset the PIN for ${currentRep.name}?`)) return;
  const btn = document.getElementById('reset-rep-pin-btn');
  const origText = btn.textContent;
  btn.textContent = 'Resetting...';
  btn.disabled = true;
  try {
    const tempPin = await window.AppAuth.resetRepPin(currentRep.repId);
    alert(`Success!\n\nThe new temporary PIN for ${currentRep.name} is: ${tempPin}\n\nThey will be forced to choose a new PIN upon their next login.`);
  } catch (err) {
    alert('Failed to reset PIN: ' + (err.message || JSON.stringify(err)));
  } finally {
