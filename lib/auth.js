...
export async function resetRepPin(targetRepId) {
  const { data, error } = await supabase.functions.invoke('reset-rep-pin', {
    body: { targetRepId }
  });
  if (error) throw error;
...
window.AppAuth = { login, logout, restoreSession, changePin, fetchRepForViewAs, updateRepAdmin, deriveLoginAlias, resetRepPin };
